# Integrating BlockBatchClearing

## From Solidity

```solidity
import {BlockBatchClearingHook} from "block-batch-clearing/src/hooks/BlockBatchClearingHook.sol";
...
const hook = getHook("block-batch-clearing");
const address = hookAddress("block-batch-clearing", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "BlockBatchClearing"
...
More at https://block-batch-clearing.pages.dev.
