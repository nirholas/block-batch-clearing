...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(BlockBatchClearingHook).creationCode, abi.encode(IPoolManager(address(manager)), 30, "Batch LP", "BATCH-LP")
        );
        hook = new BlockBatchClearingHook{salt: salt}(IPoolManager(address(manager)), 30, "Batch LP", "BATCH-LP");
