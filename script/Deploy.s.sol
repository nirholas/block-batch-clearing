...
/**
 * @title DeployBlockBatchClearing
 * @notice Deploys BlockBatchClearing deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployBlockBatchClearing is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(BlockBatchClearingHook).creationCode;
        bytes memory constructorArgs = abi.encode(manager, _swapFeeBps, name_, symbol_);
        ...
        console2.log("BlockBatchClearing ", predicted);
        ...
        vm.startBroadcast();
        BlockBatchClearingHook hook = new BlockBatchClearingHook{salt: salt}(manager, _swapFeeBps, name_, symbol_);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("BlockBatchClearing")),
